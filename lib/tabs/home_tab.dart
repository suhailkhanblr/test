...
import 'package:Duet_Classified/widgets/menudrawer.dart';
...
//! # native_admob_flutter: ^1.5.0+1 deprecated
  ...
  Widget build(BuildContext context) {
    final langPack = Provider.of<Languages>(context, listen: true).selected;
    final languageCode = Provider.of<Languages>(context).getLanguageCode;
    ...
    return Scaffold(
      ...
      key: scaffoldKey,
      drawer: (CurrentUser.textDirection == TextDirection.ltr)
          ? MenuDrawer()
          : SizedBox.shrink(),
      endDrawer: (CurrentUser.textDirection == TextDirection.rtl)
          ? MenuDrawer()
          : SizedBox.shrink(),
      appBar: AppBar(
        toolbarHeight: 80,
        ...
        title: InkWell(
          ...
          child: Row(
            ...
            children: [
              ...
              Flexible(
                child: Text(
                  (CurrentUser.location.name.isEmpty)
                      ? (CurrentUser.location.cityName.isEmpty)
                          ? langPack['Choose Location']!
                          : "${CurrentUser.location.cityName}, ${CurrentUser.location.stateName} , ${CurrentUser.location.countryName}"
                      : CurrentUser.location.name,
                  textAlign: (CurrentUser.textDirection == TextDirection.ltr)
                      ? TextAlign.left
                      : TextAlign.right,
              ...
              //   ),
              //   label: Text(
              //     CurrentUser.location.name ?? CurrentUser.location.cityName,
              //     overflow: TextOverflow.ellipsis,
              //     maxLines: 1,
              //     style: TextStyle(
              //         color: Colors.black,
                      ...
                      if (firstBuild)
                        FutureBuilder(
                          ...
                          builder: (context, AsyncSnapshot snapshot) {
                            ...
                            return Text(
                                langPack['No ads available in this location']!);
                          },
                        ),
