...
class APIHelper extends ChangeNotifier {
  ...
  static const BASE_URL = "test.vendido.in"; // Put Your Base Url here.
  ...
  static const UNENCODED_PATH = 'api/index.php';
  static const UNENCODE_PATHAAPIONLY = "api/";
  static const LOGIN_WITH_MOBILE = "mobile_number_login";
  static const CHECK_USER = "check_user";
  ...
  static const PAY_U_HASH_URL = "api/moneyhash.php";
  static const IMAGE_URL = BASE_URL + "/media_storage/profile/";
  static const IMAGE_URL_SMALL = BASE_URL + "media_storage/profile/small_";
  static const PRODUCT_IMAGE_URL = BASE_URL + "media_storage/products/";
  ...
  static const PREF_FILE = "classified";
  static const SUPPORT_EMAIL = "support@" + BASE_URL; // Put your Support Email Here
  static const PRODUCT_LOADING_LIMIT = "16";
  static const PRODUCT_LOADING_OFFSET = 8;
  static const PRODUCT_STATUS = "active";
  static const CHECK_USER_POSTLIMIT = "check_user_post_limit";
...
*/
  ...
  Future autoLoginUser({
    ...
    final url = Uri.https(
        BASE_URL, UNENCODE_PATHAAPIONLY, {'action': REGISTER_USER_URL});
    print(url);
        ...
        body: (
            {
              ...
              "mobile_number": userDTO.mobileNumber,
  ...
  Future loginUserUsingUsername({
    ...
    final url = Uri.https(BASE_URL, UNENCODE_PATHAAPIONLY,
        {'action': LOGIN_URL, 'username': username, 'password': password});
    print(url);
  ...
  Future loginUserUsingEmail({@required email, @required password}) async {
    final url = Uri.https(BASE_URL, UNENCODE_PATHAAPIONLY,
        {'action': LOGIN_URL, 'username': email, 'password': password});
    print(url);
    final response = await http.get(url);
    if (response.statusCode == 200) {
      final decodedResponse = await json.decode(response.body);
  ...
  Future<bool> checkUser(UserDTO userDTO) async {
    bool isValid = false;

    final url =
    Uri.https(BASE_URL, UNENCODE_PATHAAPIONLY, {'action': CHECK_USER});
    ...
    if (response.statusCode == 200) {
      final decodedResponse = await json.decode(response.body);
      if (decodedResponse['status'] == 'valid') {
        isValid = true;
      }
    } else {
      isValid = false;
  ...
  Future<bool> registerValidUser(UserDTO userDTO) async {
    bool result = false;
    final url = Uri.https(
        BASE_URL, UNENCODE_PATHAAPIONLY, {'action': REGISTER_USER_URL});
    print(url);
  ...
  Future<String> loginUserUsingMobileNumber(
  ...
  Future<bool> forgetPassword({@required email}) async {
    bool isFound = false;
    final url = Uri.https(BASE_URL, UNENCODE_PATHAAPIONLY, {
    ...
    if (response.statusCode == 200) {
      ...
      if (decodedData['status'] != null && decodedData['status'] == "success") {
        isFound = true;
      }
    } else {
      throw Exception('Error');
  ...
  Future<bool> checkUserPostLimit({userId}) async {
    bool isExceed = false;
    final url = Uri.https(BASE_URL, UNENCODED_PATH, {
      'action': CHECK_USER_POSTLIMIT,
    });
    try {
      final response = await http.get(url);
      if (response.statusCode == 200) {
        final decodedData = await json.decode(response.body);
        //{"status":"success","message":"exceeded"}
        if (decodedData['status'] != null &&
            decodedData['status'] == "success" &&
            decodedData['message'] == "exceeded") {
          isExceed = true;
        }
      } else {
        throw Exception('Error');
  ...
  Future fetchFeaturedAndUrgentProducts(
      {required Location userLocation}) async {
    final String url =
  ...
  Future<List<dynamic>> fetchLanguagePack() async {
    final url = Uri.https(BASE_URL, UNENCODED_PATH, {
      'action': FETCH_LANGUAGE_PACK_URL,
    });
    final response = await http.get(url);
    if (response.statusCode == 200) {
      final decodedData = languagePackModelFromJson(response.body);
      print("$decodedData");
      return decodedData;
    } else {
      throw Exception('Error');
